...
- `pool_size=1` 的正常顺序请求吞吐不低于旧实现的 95%。
...
每个 checkpoint 必须是可独立验证、可追加修正的原子提交。提交信息必须带 trailer：

```text
Actor-Checkpoint: A03
```
...
- 提交本文档和 `ACTOR_REFACTOR_PROGRESS.md`。
- 运行完整现有测试，记录基线。
...
- 在同一个 finalization commit 中加入 result manifest 并删除临时 `ACTOR_REFACTOR_PROGRESS.md`，trailer 使用 `Actor-Checkpoint: FINAL`。
...
实施期间使用仓库根目录临时文件 `ACTOR_REFACTOR_PROGRESS.md`。聊天上下文不是进度来源；Git 和账本才是。FINAL finalization 后账本由永久 `ACTOR_REFACTOR_RESULT.md` 接替。
...
- commit trailer 使用 `Actor-Checkpoint: Axx`。
...
- spec revision、spec commit、base SHA、final local/remote SHA
- work branch、draft PR URL 和最终 CI run URL/status
- A00-A09/FINAL checkpoint commit 表
- 最终架构与公开兼容性变化摘要
